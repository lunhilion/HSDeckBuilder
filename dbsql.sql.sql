...
(1, b'1', b'1', 'admin', 'admin', '2016-12-06 10:43:17'),
(2, b'1', b'0', 'seuperuser', 'superuser', '2016-12-06 10:43:18'),
(3, b'0', b'0', 'user', 'user', '2016-12-06 10:43:19');
...
BEGIN
  DECLARE `nMazzi` int(11);
  SELECT COUNT(`IDutente`) INTO nMazzi 
  FROM mazzo 
  WHERE `IDutente`=`IDutenteMazzo`;
  DECLARE `premiumuser` bit(1) DEFAULT b'0';
  SELECT premium INTO premiumuser 
  FROM utente u
  WHERE u.IDutente=IDutenteMazzo;
  IF (nMazzi<3 OR premiumuser=b'1') THEN
...
--Query per sapere ID e NomeUtente degli utenti iscritti da almeno un anno, che abbiano creato almeno un mazzo

SELECT IDutente,NomeUtente
FROM(utente LEFT JOIN mazzo ON IDutente=IDUtente)
WHERE DataRegistrazione+31536000< CURRENT_TIMESTAMP;

--Query per sapere ID e NomeUtente degli utentipremium che abbiano davvero approfittato del fatto di essere premium
--(ovvero hanno creato almeno 3 mazzi)

SELECT IDUtente,u.NomeUtente
FROM utente AS u RIGHT JOIN (SELECT IDutente, count(*) as nMazzi FROM mazzo GROUP BY IDutente) AS q ON u.IDUtente=q.IDutente;
WHERE q.nMazzi>2 AND u.utentepremium =1;

--Query per sapere NomeUtente di utenti che hanno creato almeno un mazzo con la classe paladin o non hanno creato alcun mazzo
...
END IF
END
--in teoria se la condizione dell'if è soddisfatta la entry non dovrebbe essere aggiunta nella tabella componente mazzo in quanto si tratta di trigger before
