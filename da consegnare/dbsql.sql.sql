DROP DATABASE IF EXISTS `HSDeckBuilder`;

CREATE DATABASE `HSDeckBuilder`;
USE `HSDeckBuilder`;
...
('Gladiators Longbow', 7, 5, 2, 'Epic', 'Hunter', 'Weapon'),
...
('Lights Justice', 1, 1, 4, 'Free', 'Paladin', 'Weapon'),
...
('Perditions Blade', 3, 2, 2, 'Rare', 'Rogue', 'Weapon'),
...
('Shandos Lesson', 0, 0, 0, 'Common', 'Druid', 'Spell'),
...
('Sorcerers Apprentice', 2, 3, 2, 'Common', 'Mage', 'Minion'),
...
INSERT INTO `utente` (`IDUtente`, `premium`, `admin`, `Username`, `Password`, `DataRegistrazione`) VALUES
(1, 1, 1, 'admin', 'admin', '2016-12-06 10:43:17'),
(2, 1, 0, 'seuperuser', 'superuser', '2016-12-06 10:43:18'),
(3, 0, 0, 'user', 'user', '2016-12-06 10:43:19');

-- --------------------------------------------------------

--
-- Procedure e Query (6) FINITE
--
...
DROP PROCEDURE IF EXISTS promuovi_utente //
CREATE PROCEDURE promuovi_utente (IN IDdaPromuovere int(11))
BEGIN
  UPDATE `utente`
  SET `premium` = '1'
...
-- Procedura per inserire un mazzo relativo ad utente, se l'utente non è premium e ha già raggiunto i mazzi massimi creabili non farà nulla

DELIMITER //

DROP PROCEDURE IF EXISTS `aggiungi_mazzo` //
CREATE PROCEDURE `aggiungi_mazzo` (IN `IDutenteMazzo` int(11), IN `NomeMazzoDaCreare` varchar(20), IN `classeMazzo` varchar(20))
BEGIN
  DECLARE nMazzi int(11) DEFAULT 0;
  SELECT COUNT(IDutente=IDutenteMazzo) INTO nMazzi FROM mazzo GROUP BY IDutente;
  DECLARE Puser tinyint(1) DEFAULT '0';
    SELECT premium INTO Puser FROM utente WHERE IDUtente=IDutenteMazzo;
  IF (nMazzi<3 OR Puser=1) THEN
...
-- Query per sapere ID e Username degli utenti iscritti da almeno un anno, che abbiano creato almeno un mazzo

SELECT utente.IDUtente,Username
FROM utente LEFT JOIN mazzo ON utente.IDUtente=mazzo.IDutente
WHERE DataRegistrazione+31536000< CURRENT_TIMESTAMP;

-- Query per sapere ID e Username degli utentipremium che abbiano approfittato del fatto di essere premium
-- (ovvero hanno creato almeno 3 mazzi)

SELECT utente.IDUtente,utente.Username
FROM utente RIGHT JOIN (SELECT IDutente, COUNT(*) AS nMazzi FROM mazzo GROUP BY IDutente)AS q ON utente.IDUtente=q.IDutente;
WHERE (nMazzi>2 AND utente.utentepremium=1);

-- Query per sapere Username di utenti che hanno creato almeno un mazzo con la classe paladin o non hanno creato alcun mazzo

SELECT Username
FROM utente JOIN mazzo ON IDUtente=IDutente
WHERE classeMazzo="paladin"
UNION
SELECT Username
FROM utente 
WHERE IDUtente NOT IN (SELECT IDutente FROM mazzo);

-- Query per mostrare IDUtente, Username e numero di mazzi creati da ogni utente.

SELECT IDutente, Username, COUNT(*) AS nMazzi 
FROM mazzo JOIN utente ON utente.IDUtente=mazzo.IDutente
GROUP BY IDutente  
...
-- Trigger per controllare se una componente mazzo inserita faccia parte della stessa classe del mazzo nella quale è inserita.

DROP TRIGGER IF EXISTS `controllaClasse`;
CREATE TRIGGER `controllaClasse` BEFORE INSERT ON `componentemazzo`
FOR EACH ROW BEGIN
DECLARE classeC varchar(40);
SELECT Classe INTO classeC FROM carta WHERE new.Carta=Nome;
DECLARE classeM varchar(40);
SELECT ClasseMazzo INTO classeM FROM mazzo WHERE new.IDmazzo=IDmazzo
IF classeC!=ClasseM AND classeC!='Neutral' THEN
CALL raise_error;
END IF
END
-- se la condizione dell'if è soddisfatta la entry non viene aggiunta nella tabella componente mazzo in quanto si tratta di trigger before.

-- trigger per controllare che la quantità di carte della stesso tipo aggiunta in un mazzo sia 2 e maggiore di 0.

DROP TRIGGER IF EXISTS `controllaQuantità`;
CREATE TRIGGER `controllaQuantità` AFTER INSERT ON `componentemazzo`
FOR EACH ROW BEGIN
IF new.QuantitàCarta>2 THEN
new.QuantitàCarta=2;
END IF;
IF new.QuantitàCarta<1 THEN
new.QuantitàCarta=1;
END IF;
END;
...
-- Funzione che restituisce il numero di utenti attualmente registrati

DROP FUNCTION IF EXISTS ContaUtenti
CREATE FUNCTION ContaUtenti() RETURNS varchar(255)
BEGIN
  DECLARE str varchar(255) DEFAULT 'Sono stati creati ';
  DECLARE nu int(11);
  SELECT COUNT(IDUtente) INTO nu FROM utente;
  RETURN CONCAT(str,nu,' utenti.');
END;

DROP FUNCTION IF EXISTS ContaMazzi
CREATE FUNCTION ContaMazzi() RETURNS varchar(255)
BEGIN
  DECLARE str varchar(255) DEFAULT 'Sono stati creati ';
  DECLARE nu int(11);
  SELECT COUNT(IDmazzo) INTO nu FROM mazzo;
  RETURN CONCAT(str,nu,' mazzi.');
END;